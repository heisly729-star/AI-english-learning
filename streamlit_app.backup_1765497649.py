...
def generate_image_with_gemini(word):
    """Google Gemini API를 사용하여 단어의 시각적 표현 생성
    
    Args:
        word (str): 표현할 단어
        
    Returns:
        dict with 'type', 'content', 'description'
    """
    try:
        # Gemini를 사용하여 단어에 대한 상세한 시각적 설명 생성
        model = genai.GenerativeModel('gemini-pro')
        
        prompt = f"""Create a detailed visual description of "{word}" suitable for a children's educational game.
        Describe it in 2-3 sentences focusing on:
        - Key visual characteristics
        - Colors and shapes
        - What makes it recognizable
        
        Keep it simple and vivid for kids to imagine."""
        
        response = model.generate_content(prompt)
        description = response.text.strip()
        
        # 단어와 설명을 반환 (실제 이미지 대신 텍스트 기반 표현)
        return {
        ...
        st.error(f"AI 설명 생성 오류: {e}")
        # 폴백: 기본 설명 반환
        return {
            'type': 'text_description',
            'content': word,
            'description': f"Think about what a {word} looks like!"
        }
...
def show_step3_activity(selected_mission):
    ...
    mission_title_map = {
        ...
        "mystery_20_questions": "🕵️ 미스터리 스무고개",
        "writer": "✍️ 베스트셀러 작가"
    }
    
    st.subheader(mission_title_map.get(selected_mission, "활동"))
    
    if selected_mission == "image_detective":
        st.write("🎨 **그림 설명을 읽고 단어를 맞춰보세요!**")
        
        # Gemini API 설정 확인
        if not configure_gemini():
            st.error("Gemini API 설정에 실패했습니다. 관리자에게 문의하세요.")
            return
        
        # 세션 상태 초기화
        if "detective_data" not in st.session_state:
            st.session_state.detective_data = None
            st.session_state.detective_word = None
            st.session_state.detective_options = []
        
        # 이미지 설명 및 단어 생성
        if st.session_state.detective_word is None:
            # 지문에서 랜덤 단어 선택 (예시: 간단한 단어들)
            sample_words = ["astronaut", "dog", "cat", "tree", "house", "car", "sun", "moon", "flower", "bird", 
                          "elephant", "butterfly", "rainbow", "mountain", "ocean", "guitar", "pizza", "balloon"]
            ...
            # AI 설명 생성
            with st.spinner("AI가 단어의 힌트를 만들고 있어요..."):
                data = generate_image_with_gemini(selected_word)
                st.session_state.detective_data = data
        
        # AI 생성 설명 표시
        if st.session_state.detective_data:
            data = st.session_state.detective_data
            ...
            <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); 
                ...
                <div style="font-size: 24px; font-weight: bold; margin-bottom: 15px;">AI의 힌트</div>
                <div style="font-size: 18px; line-height: 1.6; background: rgba(255,255,255,0.1); 
            ...
            st.warning("AI 힌트 생성에 실패했습니다.")
            if st.button("🔄 다시 생성하기", key="regenerate_hint"):
                st.session_state.detective_word = None
                st.rerun()
            return
        ...
        for idx, option in enumerate(st.session_state.detective_options):
            with cols[idx]:
                if st.button(f"**{option}**", key=f"option_{idx}", use_container_width=True):
                    # 정답 확인
                    if option == st.session_state.detective_word:
                        st.session_state.activity_answer = option
                        st.session_state.activity_score = 100
                        st.success("🎉 정답입니다!")
                    else:
                        st.session_state.activity_answer = option
                        st.session_state.activity_score = 30
                        st.error(f"❌ 틀렸습니다. 정답은 '{st.session_state.detective_word}'였습니다.")
        ...
        if st.button("정답 제출하기", use_container_width=True, key="submit_mystery"):
            ...
            st.session_state.activity_score = 70
            st.session_state.step = 4
            st.rerun()
    
    elif selected_mission == "writer":
        st.write("✍️ **뒷이야기를 상상해서 써보세요!**")
        st.caption("(200자 이상 작성 권장)")
        story = st.text_area(
            "이야기 작성",
            height=200,
            placeholder="뒷이야기를 상상해서 입력하세요...",
            key="writer_story"
        )
