...
def show_step4_report(quiz_score, activity_score, mission_title):
    ...
    st.subheader("📊 오늘의 학습 요약")
        ...
        # 기본 피드백
        feedback_lines = [
            f"아주 잘했어요! 당신은 '{mission_title}' 미션을 완료했습니다.",
            "",
            f"총 점수 {total_score}점을 획득했습니다. 계속 열심히 공부하면 더 좋은 결과를 얻을 수 있을 거예요!",
            "",
            f"- 퀴즈 점수: {quiz_score}점",
            f"- 활동 점수: {activity_score}점"
        ]
    
        # 이미지 탐정 미션 오답 유형 분석
        if mission_title == "🎨 이미지 탐정" and hasattr(st.session_state, "detective_answer_type"):
            answer_type = st.session_state.detective_answer_type
            feedback_lines.append("")
            feedback_lines.append("**📈 상세 분석:**")
        
            if answer_type == "correct":
                feedback_lines.append("✅ 정답을 정확히 맞췄습니다! 단어와 이미지를 잘 연결했어요.")
            elif answer_type == "semantic":
                feedback_lines.append("🔍 **의미적 오답**: 비슷한 의미의 단어를 선택했어요.")
                feedback_lines.append("   💡 **학습 팁**: 단어의 정확한 의미 차이를 공부해보세요. 비슷해 보이지만 다른 뜻을 가진 단어들이 많아요!")
            elif answer_type == "spelling":
                feedback_lines.append("📝 **철자적 오답**: 철자가 비슷한 단어를 선택했어요.")
                feedback_lines.append(f"   💡 **학습 팁**: 단어를 소리 내어 읽고 철자를 주의 깊게 확인해보세요. '{st.session_state.get('detective_wrong_answer', '')}' vs '{st.session_state.get('detective_word', '')}'의 철자 차이를 눈여겨보세요!")
            elif answer_type == "random":
                feedback_lines.append("🤔 **랜덤 오답**: 전혀 관계없는 단어를 선택했어요.")
                feedback_lines.append("   💡 **학습 팁**: 이미지를 더 자세히 관찰해보세요. 그림 속 힌트들을 놓치지 마세요!")
